...
async function getMedia (value) {
  ...
  await fetch('./data/photographers.json')
    ...
    .then((data) => {
      ...
      if (value === 'date') { displayDataGaleryzer(sortByDate(mediazer)) }
      dropDownTitle = 'Date'
      if (value === 'likes') { displayDataGaleryzer(sortByLike(mediazer)) }
      dropDownTitle = 'Popularité'
      if (value === 'title') { displayDataGaleryzer(sortByTitle(mediazer)) }
      dropDownTitle = 'Titre'
    })
}
